  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Koror Reef Supply Co"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Koror Reef Supply Co" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest reserved-sector-violation-is-held-and-unoverridable
  (testing "declared :seeks-fiac? true in a §105 full-reservation sector without all-citizen ownership -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:reserved-sector-violation} (-> (store/ledger db) last :basis)))
...
(deftest pgst-registration-unverified-is-held-and-unoverridable
  (testing "unverified PGST registration when required -> HARD hold"
    (let [[db actor] (fresh)
      ...
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:pgst-registration-unverified} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))
