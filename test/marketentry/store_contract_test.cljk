...
(defn- exercise [s]
  (store/commit-record! s {:effect :engagement/upsert
                           :value {:id "eng-x" :operator "X LLC" :jurisdiction "PLW"
  ...
  (store/commit-record! s {:effect :assessment/set
                           :path ["eng-x"]
                           :payload {:jurisdiction "PLW" :checklist ["a"] :spec-basis "x"}})
...
(deftest mem-and-datomic-parity
  (let [mem (store/seed-db)
    ...
    (is (= (:seeks-fiac? (:engagement m)) (:seeks-fiac? (:engagement d))))
    (is (= (:business-sector (:engagement m)) (:business-sector (:engagement d))))
    (is (= (:all-owners-citizens? (:engagement m)) (:all-owners-citizens? (:engagement d))))
