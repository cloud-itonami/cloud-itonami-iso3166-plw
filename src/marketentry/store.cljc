(ns marketentry.store
  "SSoT for the Palau (PLW) market-entry compliance actor, behind a
  ...
  `:seeks-fiac?` / `:business-sector` / `:has-citizen-owner?` /
  `:all-owners-citizens?` are ground truth for the
  `reserved-sector-violation?` flagship check (Foreign Investment Act,
  PNCA Title 28 §105); `:requires-pgst-registration?` /
  `:pgst-registration-verified?` are ground truth for the conditional
  Palau Goods and Services Tax check (Title 40 PNCA §1201 et seq., RPPL
  11-11).

  Uses `langchain-store.core` for the shared EDN-blob codec + identity
  schema + event-log read/append helpers (ADR-2607141600) rather than
  hand-rolling `enc`/`dec*` again.
...
(defn demo-data
  ...
  PNCA Title 28 §105); `:requires-pgst-registration?` /
  `:pgst-registration-verified?` are ground truth for the conditional
  PGST check (Title 40 PNCA §1201 et seq., RPPL 11-11)."
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Koror Reef Supply Co" :portal "palaugov.pw/rfp"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :seeks-fiac? false :business-sector :unrestricted-services
             :has-citizen-owner? true :all-owners-citizens? true
             :requires-pgst-registration? true :pgst-registration-verified? true
             :drafted? false :submitted? false
             :jurisdiction "PLW" :status :intake}
    "eng-2" {:id "eng-2" :operator "Atlantis LLC" :portal "palaugov.pw/rfp"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :seeks-fiac? false :business-sector :unrestricted-services
             :has-citizen-owner? true :all-owners-citizens? true
    ...
    "eng-3" {:id "eng-3" :operator "Ngerulmud Builders Inc" :portal "palaugov.pw/rfp"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             ...
             :drafted? false :submitted? false
             :jurisdiction "PLW" :status :intake}
    "eng-4" {:id "eng-4" :operator "Rock Islands Trading Ltd" :portal "palaugov.pw/rfp"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             ...
             :drafted? false :submitted? false
             :jurisdiction "PLW" :status :intake}
    "eng-5" {:id "eng-5" :operator "Malakal Cold Chain LLC" :portal "palaugov.pw/rfp"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :seeks-fiac? false :business-sector :unrestricted-services
             :has-citizen-owner? true :all-owners-citizens? true
             :requires-pgst-registration? true :pgst-registration-verified? false
             :drafted? false :submitted? false
             :jurisdiction "PLW" :status :intake}}})
...
(defn- engagement->tx [{:keys [id operator portal base-fee monthly-rate monitoring-months claimed-fee
                               seeks-fiac? business-sector
                               has-citizen-owner? all-owners-citizens?
                               requires-pgst-registration? pgst-registration-verified?
                               drafted? submitted?
  ...
  (cond-> {:engagement/id id}
    ...
    (some? seeks-fiac?)                   (assoc :engagement/seeks-fiac? seeks-fiac?)
    business-sector                       (assoc :engagement/business-sector business-sector)
    (some? has-citizen-owner?)            (assoc :engagement/has-citizen-owner? has-citizen-owner?)
    (some? all-owners-citizens?)          (assoc :engagement/all-owners-citizens? all-owners-citizens?)
    (some? requires-pgst-registration?)   (assoc :engagement/requires-pgst-registration? requires-pgst-registration?)
    (some? pgst-registration-verified?)   (assoc :engagement/pgst-registration-verified? pgst-registration-verified?)
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/portal :engagement/base-fee :engagement/monthly-rate
   ...
   :engagement/seeks-fiac? :engagement/business-sector
   :engagement/has-citizen-owner? :engagement/all-owners-citizens?
   :engagement/requires-pgst-registration? :engagement/pgst-registration-verified?
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :portal (:engagement/portal m)
     ...
     :seeks-fiac? (boolean (:engagement/seeks-fiac? m))
     :business-sector (:engagement/business-sector m)
     :has-citizen-owner? (boolean (:engagement/has-citizen-owner? m))
     :all-owners-citizens? (boolean (:engagement/all-owners-citizens? m))
     :requires-pgst-registration? (boolean (:engagement/requires-pgst-registration? m))
     :pgst-registration-verified? (boolean (:engagement/pgst-registration-verified? m))
