...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== engagement/intake eng-1 (PLW, clean) ==")
    ...
    (println "== jurisdiction/assess eng-4 (sets up reserved-sector-violation) ==")
    ...
    (println "== filing/submit eng-4 (reserved-sector-violation -> HARD hold) ==")
    (println (exec-op actor "t9" {:op :filing/submit :subject "eng-4"} operator))

    (println "== jurisdiction/assess eng-5 (sets up pgst-registration-unverified) ==")
    ...
    (println "== filing/submit eng-5 (pgst-registration-unverified -> HARD hold) ==")
