(ns marketentry.marketentryllm
  "MarketEntry-LLM client -- the *contained intelligence node* for the
  Palau (PLW) public-sector market-entry compliance actor.
...
(defn- propose-submit
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "seeks-fiac?=" (:seeks-fiac? e)
                        " business-sector=" (:business-sector e)
                        " has-citizen-owner?=" (:has-citizen-owner? e)
                        " all-owners-citizens?=" (:all-owners-citizens? e)
                        " requires-pgst-registration?=" (:requires-pgst-registration? e)
                        " pgst-registration-verified?=" (:pgst-registration-verified? e)
     ...
     :confidence (if (and e
                          (or (not (:requires-pgst-registration? e))
                              (:pgst-registration-verified? e)))
                   0.9 0.3)}))
